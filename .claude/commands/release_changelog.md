레포 루트 `CHANGELOG.md` 의 `## Unreleased` 섹션을 릴리스 버전으로 승격한다.

## 원칙

- **릴리스 직전**에만 실행.
- 이 레포의 기본 브랜치는 **`dev`** 다. `dev` 또는 release 브랜치에서 실행 권장.
...
`CHANGELOG.md` 에서: `## Unreleased` 존재/항목 여부, 최신 릴리스 버전 확인.
...
> 참고: 이 프로젝트는 README 상 "버전 2" 진행 중이다 (2025-11-04 시작).
> 첫 릴리스 번호를 정할 때 사용자에게 이 맥락을 확인한다.
...
# 전용 릴리스 티켓이 있으면: [{PREFIX}-{번호}] docs : release changelog v{version}
```
