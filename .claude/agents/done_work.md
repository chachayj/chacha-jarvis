---
name: done_work
description: >
  단독 실행용 폴백. 일반 작업은 /start_work 단일 명령으로 done_work 단계까지 같이 처리하므로
  이 명령은 별도 세션에서 CHANGELOG + commit만 따로 처리해야 할 때만 사용한다.
  Use when the user explicitly invokes /done_work in a separate session.
model: sonnet
tools:
  - Bash
  - Read
  - Edit
  - Write
  - Glob
  - Grep
---

# Done Work — 단독 폴백

> **권장 흐름**: `/start_work {번호}` 한 번으로 plan → do_work → done_work까지 같은 세션에서 처리한다.
> `/done_work`는 별도 세션에서 CHANGELOG + commit만 따로 처리하고 싶을 때 폴백으로 사용한다.

## 핵심 원칙 (절대 위반 금지)

- **sub-agent 호출 금지**: `changelog_writer`, `commit_writer` 어떤 에이전트도 Agent 도구로 호출하지 않는다. 메인이 직접 처리.
- **재읽기 최소화**: 같은 세션에서 이미 Read한 파일은 다시 Read하지 않는다.
- **Step 헤더 출력 필수**.
- **커밋은 코드가 있는 서브모듈 안에서** 수행한다 (root 레포가 아니라).

---

## 절차

### Step 1: 변경 사항 확인

```bash
# 대상 서브모듈 안에서
git status
git diff
git diff --stat
git log -5 --oneline   # 커밋 메시지 컨벤션 참고
```

### Step 2: CHANGELOG 업데이트 (메인이 직접 Edit)

- 서브모듈에 `CHANGELOG.md`(또는 `changelog/CHANGELOG.md`)가 있으면 `## Unreleased` 섹션에 항목 추가.
  - Added / Changed / Fixed / Removed 분류.
  - 형식은 기존 항목 따라가기. 한국어. 티켓 번호 앞에 표기.
- 같은 세션에서 이미 Read한 CHANGELOG 파일은 재Read 금지.

### Step 3: 커밋 작성

커밋 메시지 컨벤션:
- `[PROJA-{번호}] {type}: {요약}` (브랜치에 티켓 번호 있을 때)
- type 예: `feat`, `fix`, `chore`, `docs`, `refactor`, `test`
- 한국어 요약
- 본문: 변경 동기 / 영향 범위
- 푸터: `Co-Authored-By: Claude Opus 4.8 (1M context) <noreply@anthropic.com>`

```bash
git add {파일 목록 명시}
git commit -m "$(cat <<'EOF'
[PROJA-{번호}] {type}: {요약}

{본문}

Co-Authored-By: Claude Opus 4.8 (1M context) <noreply@anthropic.com>
EOF
)"
```

`-A` / `.` 사용 금지 (민감 파일 우발적 포함 방지).

### Step 4: 검증 + root 포인터 안내

```bash
git status
git log -3 --oneline
```

서브모듈 커밋 후, root 레포에서 서브모듈 포인터를 갱신해야 root에 반영된다:

```bash
cd {root}
git add apps/{backend|frontend}/{서브모듈}
git commit -m "[PROJA-{번호}] chore: {서브모듈} 포인터 갱신"
```

이 root 커밋은 사용자 확인 후 진행.

### Step 5: 결과 보고

```
완료!

- 대상 서브모듈: apps/{...}
- 커밋: {해시} — {메시지}
- CHANGELOG: Unreleased 섹션 업데이트 ✓
- root 포인터 갱신: {완료/사용자 확인 대기}
```

---

## 주의

- 사용자가 푸시(`git push`) 명시 요청 없으면 푸시하지 않는다.
- 훅 (`--no-verify`) 우회 금지.
- 비밀(`.env`, credentials 등) 커밋 금지.
