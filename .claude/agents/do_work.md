...
- 브랜치명에서 티켓 번호 추출 (`feature/{PREFIX}-{번호}`).
...
| 영역 | 경로 | 스택 |
|------|------|------|
| Spring 백엔드 | `backend/spring_server` | Spring Boot 3.2.6 / Java 17 / MyBatis / PostgreSQL |
| Flask 백엔드 | `backend/flask_server` | Python Flask |
| robot server | `backend/go_fiber_server` | Go 1.22.5 / Fiber / MQTT |
| 3D map 프론트 | `frontend/web/vue` | Vue 3 / TS / Vite / Pinia / Cesium |
| 음성 챗봇 | `voice-assistant` | FastAPI + faster-whisper + piper-tts/gTTS → Ollama |
| 엣지 | `embeded/{ai,ros}` | TensorFlow.js, rospy |
| DB | `postgres/{initdb,schema}` | PostgreSQL |
| 인프라 | `docker-compose.yml`, `nginx/`, `emqx/`, `shellscripts/` | 도커 컴포즈 / Nginx / EMQX |
...
빌드 산출물(`build/`, `dist/`, `node_modules/`) 직접 편집 금지. 빌드는 사용자가 직접 실행.
...
- 빌드는 사용자가 직접 실행 (`./gradlew build`, `npm run build`, `go build`, `docker compose up --build`).
- `plane-selfhost/plane-app/plane.env` 는 시크릿 파일이라 추적 제외 대상이다 — 커밋하지 않는다.
